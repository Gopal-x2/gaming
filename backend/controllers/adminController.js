...
// @desc    Verify Manual UPI Payment
...
exports.verifyManualPayment = async (req, res, next) => {
  try {
    ...
    const registration = await TournamentRegistration.findOne({ payment: payment._id });
    if (registration) {
      ...
      if (status === 'SUCCESS') {
        ...
        await Notification.create({
          user: registration.captain,
          title: 'Payment Verified',
          message: `Your payment of ₹${payment.amount} has been verified! Registration confirmed.`,
        ...
        await Notification.create({
          ...
          message: `Your payment of ₹${payment.amount} was rejected by the admin. Please contact support.`,
          type: 'PAYMENT_FAILED',
        });
      }
    }
